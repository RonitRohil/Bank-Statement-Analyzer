  ...
  return (
    <div className="mb-8">
      ...
      {named.length > 0 ? (
            ...
            return (
              ...
              >
                <div className="flex justify-between items-start mb-3">
                  <h4 className="font-semibold text-slate-800 truncate pr-2" title={name}>
                    {name}
                  </h4>
                ...
                <div className="space-y-2.5">
                  <div className="flex items-center justify-between text-sm">
                    ...
                    <span className="font-semibold text-slate-800">{inr(data.avg_amount || 0)}</span>
                  </div>

                  <div className="flex items-center justify-between text-sm">
                    <span className="text-slate-400">Total spent</span>
                    <span className="font-semibold text-slate-800">{inr(totalVol)}</span>
                  </div>
                  ...
                  <div className="flex items-center justify-between text-sm">
                    ...
                    <span className="text-slate-600 text-xs">{fmtDate(data.last_seen)}</span>
                  </div>
                </div>
              </div>
            );
        ...
        <p className="text-slate-400 text-sm">No named merchants could be identified from narrations.</p>
      )}
    </div>
  );
};
