...
export const AnalyticsCharts: React.FC<AnalyticsChartsProps> = ({ transactions, merchantInsights }) => {
  ...
  const pieData = otherValue > 0
    ? [...topMerchants, { name: 'Others', fullName: 'Others', value: otherValue }]
    : topMerchants;
  ...
  return (
    <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-8">
      ...
      <div className="bg-white p-5 rounded-xl shadow-sm border border-slate-100 lg:col-span-2">
        <h3 className="font-bold text-slate-800 text-base mb-5">Balance History</h3>
      ...
      <div className="bg-white p-5 rounded-xl shadow-sm border border-slate-100">
        ...
        <div className="flex gap-5 mb-4">
          ...
          <div>
            ...
            <p className={`text-sm font-bold ${net >= 0 ? 'text-emerald-600' : 'text-rose-500'}`}>
        ...
        <div className="h-48 w-full">
          <ResponsiveContainer width="100%" height="100%">
            <BarChart data={flowData} barCategoryGap="30%">
              ...
              <Bar dataKey="Income" fill="#10b981" radius={[4, 4, 0, 0]} barSize={52} />
              <Bar dataKey="Expense" fill="#f43f5e" radius={[4, 4, 0, 0]} barSize={52} />
            </BarChart>
          </ResponsiveContainer>
        </div>
      </div>
      ...
      <div className="bg-white p-5 rounded-xl shadow-sm border border-slate-100">
        <h3 className="font-bold text-slate-800 text-base mb-4">Top Merchants by Volume</h3>
            ...
            <>
              <ResponsiveContainer width="100%" height="100%">
                <PieChart>
                  ...
                  <Tooltip
                    formatter={(val: number, _name: string, props: any) => [inr(val), props.payload.fullName]}
                    contentStyle={{ borderRadius: '8px', border: 'none', boxShadow: '0 4px 6px -1px rgb(0 0 0 / 0.1)', fontSize: 13 }}
                  />
                  <Legend
                    iconType="circle"
              ...
              <div className="absolute inset-0 flex items-center justify-center pointer-events-none" style={{ paddingBottom: 24 }}>
                <div className="text-center">
                  <p className="text-lg font-bold text-slate-700">{pieData.length}</p>
                  <p className="text-xs text-slate-400 leading-none">merchants</p>
                </div>
              </div>
            </>
          ) : (
            <div className="flex h-full items-center justify-center">
              <p className="text-slate-400 text-sm">No named merchants detected</p>
            </div>
          )}
        </div>
      </div>
